...
export function BottomNav({ cartCount, onOpenCart, activeTab = "home" }: BottomNavProps) {
  return (
    <nav className="bottom-nav" aria-label="เมนูด่วน">
      ...
      <button type="button" data-cart-target="bottom-nav" className={`bottom-nav-item${activeTab === "cart" ? " active" : ""}`} onClick={onOpenCart} aria-label={`เปิดตะกร้า มีสินค้า ${cartCount} ชิ้น`}>
        <svg className="bottom-nav-cart-icon" viewBox="0 0 24 24" aria-hidden="true">
